<div>
    ...
    @if ($cartItems->count() > 0)
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 p-3 bg-light rounded">
            ...
            @if ($this->selectedItemsCount > 0)
                <button wire:click="deleteSelected" wire:confirm="Hapus item terpilih?"
                        ...
                        <div class="d-flex flex-column gap-2 mb-3">
                            ...
                            @if ($item->produkUkuran && $item->produkUkuran->ukuran)
                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                    <span
                                        class="badge bg-secondary bg-opacity-25 text-dark border border-secondary small py-1">
                                    ...
                                    @if ($item->produkUkuran->stok > 0)
                                        <span
                                            class="badge bg-info bg-opacity-25 text-dark border border-info small py-1">
                                            <i class="fa-solid fa-basket-shopping"></i>
                                            Stok: {{ $item->produkUkuran->stok }}
                                        </span>
                                    @else
                                        <span
                                            class="badge bg-danger bg-opacity-25 text-dark border border-danger small py-1">
                                ...
                                <div class="d-flex align-items-start">
                                    ...
                                    <div class="form-check me-3 flex-shrink-0 mt-1">
                                        <input class="form-check-input" type="checkbox"
                                            wire:model.live="selectedItems" value="{{ $item->id_keranjang }}"
                                            id="item-{{ $item->id_keranjang }}-desktop"
                                            {{ $item->produkUkuran && $item->produkUkuran->stok == 0 ? 'disabled' : '' }}>
                                    </div>
                                    ...
                                    @if ($item->produkUkuran && $item->produkUkuran->produk && $item->produkUkuran->produk->foto_utama)
                                        <img src="{{ asset($item->produkUkuran->produk->foto_utama) }}"
                                            alt="{{ $item->produkUkuran->produk->nama }}" width="80"
                                            height="80"
                                            class="me-3 rounded object-fit-cover flex-shrink-0 {{ $item->produkUkuran && $item->produkUkuran->stok == 0 ? 'opacity-50' : '' }}">
                                    @else
                                        <img src="{{ asset('images/default/default-product.png') }}"
                                            alt="Default Product" width="80" height="80"
                                            class="me-3 rounded object-fit-cover flex-shrink-0 {{ $item->produkUkuran && $item->produkUkuran->stok == 0 ? 'opacity-50' : '' }}">
                                    @endif
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="d-flex justify-content-between align-items-start mb-0">
                                            <h6 class="fw-semibold mb-0 text-break me-md-1 {{ $item->produkUkuran && $item->produkUkuran->stok == 0 ? 'text-muted' : '' }}"
                                        ...
                                        @if ($item->produkUkuran && $item->produkUkuran->ukuran)
                                            <div class="mb-3">
                                                <div class="d-flex align-items-center gap-2">
                                                    <span
                                                        class="badge bg-secondary bg-opacity-25 text-dark border border-secondary small py-2">
                                                        <i class="fas fa-ruler me-1"></i>
                                                        {{ $item->produkUkuran->ukuran->nama_ukuran }}
                                                    </span>
                                                    ...
                                                    @if ($item->produkUkuran->stok > 0)
                                                        <span
                                                            class="badge bg-info bg-opacity-25 text-dark border border-info small py-2">
                                                            <i class="fa-solid fa-basket-shopping"></i>
                                                            Stok: {{ $item->produkUkuran->stok }}
                                                        </span>
                                                    @else
                                                        <span
                                                            class="badge bg-danger bg-opacity-25 text-dark border border-danger small py-2">
                                                            <i class="fas fa-times me-1"></i>
                                                            Stok Habis
                                                        </span>
                                                    @endif
                                            ...
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <button type="button"
                                                        ...
                                                        <i class="fas fa-minus"></i>
                                                        <div wire:loading
                                                            wire:target="updateQuantity({{ $item->id_keranjang }}, -1)"
                                                            class="position-absolute top-50 start-50 translate-middle">
                        ...
                        <div class="card border-0 bg-light position-sticky" style="top: 80px;">
                            <div class="card-body">
                                ...
                                <a href="#" class="btn btn-success w-100 mb-2">
                                    Checkout ({{ $this->selectedItemsCount }} item)
                                </a>
